...
public class MovieUserDAO {
    ...
    public List<MovieUserListRes> movieChartTicketingList() {
        ...
        String sql = "WITH YES_TICKET AS ( "
    			+ "SELECT * FROM TICKETING WHERE SCREEN_INFO_IDX IN (SELECT SCREEN_INFO_IDX FROM SCREEN_INFO WHERE SCREEN_DATE = SUBDATE(CURDATE(), 19)) "
    			+ "AND TICKETING_DEL = 'N' "
    			+ "), YES_REVIEW as (  "
    ...
    public MovieDetailRes movieDetail(int movieIdx) throws Exception{
    	...
    	String sql = "WITH YES_TICKET as ( " 
    				...
    				+ "where SCREEN_INFO_IDX IN (select SCREEN_INFO_IDX from SCREEN_INFO where SCREEN_DATE = SUBDATE(CURDATE(), 19)) "
    				+ "and TICKETING_DEL = 'N' ) "
    				+ ", YES_REVIEW as ( "
    				+ "SELECT FLOOR(REVIEW_SCORE/2) as SCORE "
